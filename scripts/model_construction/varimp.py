...
if __name__ == "__main__":
    ...
    # Generate datasets and data loaders using the custom 'dataset_generation' function
    (
        numbers,
        trainset,
        valset,
        ...
        shape_data,
        shape_label,
    ) = dataset_generation(Xdata, lab, image_X)
    
    # Create a DataLoader for the test set with a large batch size and shuffling enabled
    ...
    model = torch.load(f"./{folder}/{category}10_3model").to(device)
    ...
    for tg in range(st, end):
        ...
        if str(tg) in os.listdir(outputdir):
        ...
        output_file_path = os.path.join(outputdir, f"{tg}sumed")
        ...
        # with open(os.path.join(outputdir, f"{tg}_ig.sumed"), "wb+") as file:
        #     summed_ig = np.concatenate(dumpdict["ig"]).sum(0)
        #     pickle.dump(summed_ig, file)
        #
        # with open(os.path.join(outputdir, f"{tg}_dl.sumed"), "wb+") as file:
        #     summed_dl = np.concatenate(dumpdict["dl"]).sum(0)
        #     pickle.dump(summed_dl, file)
        #
        # with open(os.path.join(outputdir, f"{tg}_ks.sumed"), "wb+") as file:
        #     summed_ks = np.concatenate(dumpdict["ks"]).sum(0)
        #     pickle.dump(summed_ks, file)
        #
        # with open(os.path.join(outputdir, f"{tg}_dls.sumed"), "wb+") as file:
